...
- Original upstream files compiled directly into the native build:
  ...
  - `src/oak_speech.c` for the real New Game provider reached from the main-menu handoff, currently proven through controls-guide page transitions and Pikachu intro pages 1 through 3
...
- Narrow host Oak Speech text/window/menu/naming/mon-sprite shims, isolated under `host_oak_speech_stubs.c` / `host_oak_speech_stubs.h`, that let unchanged upstream `oak_speech.c` link and execute its controls-guide and Pikachu-intro path without widening gameplay rewrites into the upstream file.
...
1. deepen the newly linked `src/oak_speech.c` path beyond its current controls-guide/Pikachu-intro-pages-1-through-3 boundary toward Oak Speech proper, gender selection, naming, and the eventual `CB2_NewGame` handoff
