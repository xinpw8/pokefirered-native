#!/usr/bin/env python3
"""
play_web.py -- Wrapper around play.py that fixes port + JSON + adds save_state.
Runs play.py's game loop on port 53582 with a /save/<name> endpoint.
"""
...
SAVE_DIR = "/home/spark-advantage/pokefirered-native/savestates"
...
class WebHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/" or self.path == "/index.html":
            ...
            with self.game.lock:
                info = {
                    "step": int(self.game.step_count),
                    "reward": round(float(self.game.total_reward), 2),
                    "action": ACTION_NAMES[self.game.last_action],
                    "save_msg": self.game.save_msg,
                }
                if self.game.obs is not None:
                    ...
                    info["map"] = "{}.{}".format(int(o[4]), int(o[5]))
            ...
            path = os.path.join(SAVE_DIR, name + ".ss")
            try:
                ret = binding.save_state(0, path)
                msg = f"Saved: {path} (ret={ret})"
                print(f"[SAVE] {msg}", flush=True)
                with self.game.lock:
                    self.game.save_msg = msg
            ...
            name = self.path[6:].strip("/")
            if not name:
                name = "init_players_room_2f"
            path = os.path.join(SAVE_DIR, name if name.endswith(".ss") else name + ".ss")
            try:
                ret = binding.load_state(0, path)
                msg = f"Loaded: {path} (ret={ret})"
                print(f"[LOAD-WEB] {msg}", flush=True)
                with self.game.lock:
                    self.game.save_msg = msg
            except Exception as e:
                msg = f"Load FAILED: {e}"
        ...
        elif self.path == "/list_saves":
            os.makedirs(SAVE_DIR, exist_ok=True)
            files = sorted([f for f in os.listdir(SAVE_DIR) if f.endswith(".ss")])
            self.send_response(200)
            self.send_header("Content-Type", "application/json")
            self.send_header("Access-Control-Allow-Origin", "*")
            self.end_headers()
...
<style>
  * { box-sizing: border-box; }
  ...
  .save-btn {
    font-size: 16px; padding: 10px 20px; margin: 8px; cursor: pointer;
    background: #530; color: #fc0; border: 2px solid #a60; border-radius: 8px;
  }
  .save-btn:hover { background: #a60; }
  #status { color: #aaa; margin: 8px; font-size: 14px; }
  #save_msg { color: #0f0; margin: 4px; font-size: 13px; min-height: 18px; }
...
<div style="margin-top: 12px;">
  <input id="save_name" type="text" placeholder="savestate name" value="post_oak" style="font-size:16px;padding:8px;background:#222;color:#eee;border:1px solid #555;border-radius:4px;width:200px;">
  <button class="save-btn" onclick="doSave()">SAVE STATE</button>
</div>
<div id="held"></div>
<div class="keys-hint">Keyboard: Arrows=D-pad, Z=A, X=B, Enter=Start | F5=Save State</div>
...
function doSave() {
  let name = document.getElementById("save_name").value.trim() || "quick";
  fetch("/save/" + encodeURIComponent(name))
    .then(r => r.text())
    .then(t => { document.getElementById("save_msg").textContent = t; })
    .catch(e => { document.getElementById("save_msg").textContent = "Save error: " + e; });
}
...
setInterval(function() {
  fetch("/status").then(function(r){return r.json();}).then(function(d) {
    ...
    if (d.save_msg) document.getElementById("save_msg").textContent = d.save_msg;
  }).catch(function(){});
}, 250);
</script>
</body>
...
def main():
    global _quit_flag
    so_path = "/home/spark-advantage/pokefirered-native/build/libpfr_game.so"
    ...
    print("", flush=True)
    print(f"  >>> OPEN: http://192.168.0.26:{port}/", flush=True)
    print(f"  >>> Saves go to: {SAVE_DIR}/", flush=True)
    print("", flush=True)
    ...
    while not _quit_flag:
        ...
        binding.vec_step(c_envs)
        game.step_count += 1
        game.total_reward += float(rewards[0])
